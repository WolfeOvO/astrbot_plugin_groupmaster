...
- 🤖 **LLM 兜底**：命令没记住也没关系，@Bot 说人话即可，插件注册 9 个 llm_tool 供大模型调用
...
示例：`@Bot 把他禁言十分钟`、`@Bot 踢了刚才发广告的人`、`@Bot 撤回他最近 5 条消息`、`@Bot 现在谁被禁言着`。
...
- **status 禁言列表**：依赖 NapCat `get_group_shut_list` 扩展接口，较旧版本/其他 OneBot 实现会提示不可查（警告计数与拉黑名单不受影响）
